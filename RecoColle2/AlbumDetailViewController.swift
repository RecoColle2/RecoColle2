...
//    let titleLabels: [String] = ["title1", "title2", "title3", "title4"]
        ...
        if checkArray.count == 0 {
            ...
            if flg == "add" {
                ...
                for check in checkArray {
                    ...
                    let predicate = NSPredicate(format: "id == %@", album.id!)
    ...
    @IBAction func cancel(_ sender: Any) {
        ...
        if selectFlg == "1" {
            selectFlg = "2"
            cancelButton.setTitle("calcel", for: .normal)
        }else{
            selectFlg = "1"
            cancelButton.setTitle("select", for: .normal)
...
//            cancelButton.isHidden = true
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if (segue.identifier == "toDetailViewController") {
                ...
                if flg == "add" {
                    destination.recordList = recordLists[wk_indexPath.row]
                    destination.gamenflg = "albumDetail"
                }else{
                    let album = albums[wk_indexPath.row]
                    ...
                    destination.recordList = recordLists[0]
                    destination.gamenflg = "albumDetail"
                }
        }
        if (segue.identifier == "toAlbumDetailAddViewController") {
                guard let destination = segue.destination as? AlbumDetailAddViewController else {
                    fatalError("Failed to prepare DetailViewController.")
                }
                destination.str = str
